...
- (BOOL)bannerTargetingAllowed:(CPAppBanner*)banner {
    ...
    for (CPAppBannerEventFilters *events in banner.eventFilters) {
        [self insertRecordWithBannerID:banner.id trackEventID:events.event property:events.property value:events.value relation:events.relation count:@1 createdDateTime:currentTimeStamp updatedDateTime:currentTimeStamp fromValue:events.from_value toValue:events.to_value];
    }
